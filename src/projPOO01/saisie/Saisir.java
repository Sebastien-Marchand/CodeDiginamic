...
public class Saisir {
	...
	/**
	 * Permet la saisie d'un patron
	 * nom
	 * prenom
	 * adresse
	 * ville
	 * codrepostal
	 * numero sécurité social
	 * salaire
	 */
	...
	/**
	 * Permet de saisir un salarié
	 * nom
	 * prenom
	 * adresse
	 * ville
	 * code postale
	 * numero securité social
	 * salaire
	 */
...
/**
 * Permet de saisir un intérimaire
 * nom
 * prenom
 * adresse
 * ville
...
public static void SaisirInterimaire(){
		...
		String nom, prenom, adresse, ville, codepostal="", secu = null, agence="";
		double salaire=0;
		int duree = 0;
		boolean erreurcp = true;
		boolean erreurns =true;
		...
		for(int i=0;i<5; i++) {
			...
			while(erreurns) {
				try {
					...
					System.out.println(e.getMessage()+ " "+ secu.length());
																								...
																								//saisie du nom de l'agence
			erreurns=true;
			while(erreurns) {
				try {
					System.out.println("Saisir l'agence de l'interimaire");
					agence = Menus.sc.next();
					try {
						Interimaire.CtrlSaisiAgence(agence);
					} catch (Exception e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
			...
			while(Menus.sc.hasNext()&& salaire == 0)
			{
				...
				else
				{
					...
					Menus.sc.next();
																									...
																									//Saisie de la durée
			System.out.println("Saisir la duree de l'interimaire");
			while(Menus.sc.hasNext()&& duree == 0)
			{
				if (Menus.sc.hasNextInt() && duree > 1) {
			...
			Interimaire interim = new Interimaire(nom, prenom, adresse, ville, codepostal, secu, agence , duree, salaire);
	...
	/**
	 * Permet la saisie d'un client
	 * nom
	 * prenom
	 * adresse
	 * ville
	 * code postale
	 * numero unique client
	 * 
	 */
	...
	/**
	 * Permet la saisie d'un Fournisseur
	 * nom
	 * prenom
	 * adresse
	 * ville
	 * codepostal
	 * numero unique fournisseur
	 */
	 ...
	 * @return ArrayList<Achat> liste de toute les achats
	 */
	...
	/**
	 * Permet la saisie d'une commande par l'utilisateur
	 * 
	 * intitulé de commande
	 * quantité
	 * date
	 * 
	 * @return ArrayList de commande retourne toute les commandes
	 */
